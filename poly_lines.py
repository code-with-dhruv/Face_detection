...
while True:
    ...
    if len(faces) > 0:
        ...
        for landmark in landmarks:
            ...
            # Optionally connect the regions to form a complete mask
            ...
            for region in regions.values():
                ...
                for j in range(len(points) - 1):
                    cv2.line(frame, (int(points[j][0]), int(points[j][1])), (int(points[j + 1][0]), int(points[j + 1][1])), (0, 255, 0), 1)
                cv2.line(frame, (int(points[-1][0]), int(points[-1][1])), (int(points[0][0]), int(points[0][1])), (0, 255, 0), 1)
